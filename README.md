...
Каждый микросервис использует собственную in-memory базу данных (H2) и управляет схемой независимо.
...
Для оптимизации типовых запросов и бизнес-сценариев добавлены индексы:
- индекс по `user_id` — используется при получении истории бронирований пользователя;
- индекс по `room_id` — используется при анализе загрузки номера;
- индекс по `status` — используется при фильтрации бронирований по состоянию;
- составной индекс `(start_date, end_date)` — используется при проверке пересечений дат бронирования.

Связь между `Booking` и `User` реализована как `ManyToOne`.
...
- `hotels`
- `rooms`
- `room_reservation_lock`

Для поддержки саги бронирования и идемпотентности реализованы:
- уникальный индекс по `request_id` в таблице `room_reservation_lock`;
- составной индекс `(room_id, start_date, end_date)` для эффективной проверки конфликтов дат при параллельных бронированиях.

Такая схема позволяет корректно обрабатывать конкурентные запросы и обеспечивает согласованность данных между сервисами.
---
...
В проекте используется **Spotless Maven Plugin** для обеспечения единого базового стиля кода без изменения существующего форматирования.

Spotless применяется для:
- удаления неиспользуемых импортов;
- удаления хвостовых пробелов;
- обеспечения перевода строки в конце файлов.

Форматтер **не изменяет отступы и структуру кода**, что позволяет сохранить текущий стиль проекта и избежать массовых изменений файлов.
...
Рекомендуется запускать `spotless:apply` только перед коммитом и проверять изменения в diff.